...
DEFAULT_OUT_DIR = Path("data/ring_hunter_library/icycore_subregions")


def export_subregion_lists(
    db_path: Path,
    ...
    with closing(sqlite3.connect(db_path)) as conn:
        ...
        for subregion in subregions:
            rows = conn.execute(
                """
                SELECT rank, ring_id, moi_metric, system_name, body_name, ring_name, x, y, z, theta_deg, rho_ly
                FROM icy_subregions
            ...
            path = out_dir / f"subregion_{subregion}.csv"
            with path.open("w", newline="", encoding="utf-8") as handle:
                writer = csv.writer(handle)
                writer.writerow(
                    ["rank", "ring_id", "moi_metric", "system_name", "body_name", "ring_name", "x", "y", "z", "theta_deg", "rho_ly"]
                )
                for row in rows:
                    writer.writerow(
                        [
            ...
            counts[subregion] = len(rows)
    return counts


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Export per-subregion CSV lists for IcyCore.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    counts = export_subregion_lists(
        db_path=Path(args.db),
        score_version=args.score_version,
        cohort_name=args.cohort_name,
        out_dir=Path(args.out_dir),
    )
    for subregion in sorted(counts):
        print(f"{subregion}: {counts[subregion]}")
    return 0
